...
## 索引的代价
### 空间代价
每建立一个索引，就要建立一颗B+树，而B+树有很多数据页，每个数据页的大小时32K，占用磁盘空间巨大。
### 时间代价
索引是一颗B+树，对这颗树进行增删改查时，需要对B+树进行维护。维护的过程可能造成页分裂等问题，时间耗费很多
### 执行计划
对于一条查询语句，在执行前，需要生成执行计划，执行计划会分析该条查询语句使用哪个索引效率更高，如果索引很多的话，分析效率的时间也会延长，浪费时间。
...
### 索引排序
索引可以用来代替Order By排序
Order By一般采用排序算法，但是如果建立索引时的顺序正好与要排列的顺序相同，就可以省略这个步骤。
不仅仅可以升序，也可以降序。
虽然页内时单链表，但是可以通过遍历槽的方式进行反方向遍历。
支持最左前缀原则。
### 索引分组
分组的顺序与建立索引的顺序相同时，分组操作也可以省略。
支持最左前缀原则
## 回表的代价
对于一个二级索引，执行回表操作，尽管二级索引列是有序的，但是他们的主键id数是无序的，也就是回表的顺序不是按照主键顺序的，就需要访问不同位置的数据页，就会有许多磁盘随机读io，浪费性能。


## 建立索引的原则
### 只为用于搜索、排序、分组的列创建索引
### 考虑索引列中不重复值的个数
重复的值很多时，要通过二次索引+回表查找很多次，浪费时间。
### 索引列的类型尽量小
### 为列前缀建立索引
如果存储的类型是字符串，可以只为字符串的前十个字符建立索引，这样B+树节点存储的数据就少了。
### 覆盖索引
### 让索引列以列名的方式单独出现
where key*2 > 4
key有索引也不会走
